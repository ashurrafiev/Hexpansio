...
public abstract class FarmUpgrades {
	...
	public static void init() {
		new Improvement(farm, "Fertiliser", 30, 1).maintenance(1).yield(2, 0, 0, 0);
		...
		new Improvement(farm, "Granary", 60, 1).cityUnique().maintenance(1)
			.effects(new YieldEffect.Resource(2, 0, 0, 0) {
				@Override
		...
		new Improvement(farm, "Distillery", 80, 2).cityUnique().workplaces(1).maintenance(2).yield(0, 2, 0, 0)
			.effects(new YieldEffect.Resource(0, 0, 0, 1) {
				@Override
				public int addResourceBonusYield(TokenResource resource, YieldResource res) {
					return resource==TokenResource.grain || resource==TokenResource.berries ? this.yield.get(res) : 0;
				}
				@Override
				public String getDescription() {
					return "+1 Happiness from Grain and Berries";
				}
			});
	}
